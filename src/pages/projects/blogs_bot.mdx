...
`ltn blog create relyqs blog` - create your blog - result is new blog with name `#relyqs-blog`
...
`ltn blog thread create screenshots` - create a thread on your blog \
`ltn blog thread delete` - delete this thread \
`ltn blog thread rename screencaps` - rename this thread \
`ltn blog thread slowmode 60` - set thread slowmode \
...
`ltn blogset max_threads [number]` set the maximum number of threads users can create on their blog - def: 0

`ltn blogset thread_duration [number]` set the time in minutes before a thread is archived. can be any of 60 (hour), 1440 (day), 4320 (3 days), 10000 (week) - def: 4320
